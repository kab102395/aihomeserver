...
impl KnowledgeStore {
    ...
    pub async fn upsert(&self, topic: &str, summary: &str, content: &str, tags: &str, sources: &str) -> Result<KnowledgeEntry> {
        let now = Utc::now();
        let now_str = now.to_rfc3339();

        // Check if topic already exists
        let existing: Option<(String,)> = sqlx::query_as(
            "SELECT id FROM knowledge WHERE LOWER(topic) = LOWER(?1) LIMIT 1"
        )
        .bind(topic)
        ...
        let id = if let Some((existing_id,)) = existing {
            ...
            .bind(summary).bind(content).bind(tags).bind(sources)
            .bind(&now_str).bind(&existing_id)
    ...
    pub async fn find_relevant(&self, text: &str) -> Result<Vec<KnowledgeEntry>> {
        ...
        let mut scored: Vec<(u32, KnowledgeEntry)> = all.into_iter().filter_map(|entry| {
            let topic_lower = entry.topic.to_lowercase();
            let tags_lower  = entry.tags.to_lowercase();
            let mut score: u32 = 0;
            for word in &words {
                if topic_lower.contains(word.as_str()) { score += 3; }
                if tags_lower.contains(word.as_str())  { score += 1; }
            }
    ...
    pub async fn list(&self) -> Result<Vec<KnowledgeEntry>> {
        let rows: Vec<(String, String, String, String, String, String, String, String, i64)> =
            sqlx::query_as(
        ...
        Ok(rows.into_iter().filter_map(|(id, topic, summary, content, tags, sources, created_str, updated_str, version)| {
            let created_at = DateTime::parse_from_rfc3339(&created_str).ok()?.with_timezone(&Utc);
            let updated_at = DateTime::parse_from_rfc3339(&updated_str).ok()?.with_timezone(&Utc);
            Some(KnowledgeEntry { id, topic, summary, content, tags, sources, created_at, updated_at, version })
        }).collect())
    }
    ...
    pub async fn get(&self, id: &str) -> Result<Option<KnowledgeEntry>> {
        ...
        Ok(row.and_then(|(id, topic, summary, content, tags, sources, created_str, updated_str, version)| {
            let created_at = DateTime::parse_from_rfc3339(&created_str).ok()?.with_timezone(&Utc);
            let updated_at = DateTime::parse_from_rfc3339(&updated_str).ok()?.with_timezone(&Utc);
            Some(KnowledgeEntry { id, topic, summary, content, tags, sources, created_at, updated_at, version })
        }))
