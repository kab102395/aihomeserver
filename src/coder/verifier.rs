...
pub fn verify(
    ...
    let missing_files: Vec<String> = manifest
        ...
        .iter()
        .filter(|rel| {
            let full = ws.join(rel.trim_start_matches('/'));
    ...
    for artifact_path in &manifest.expected_artifacts {
        let full = ws.join(artifact_path.trim_start_matches('/'));
...
fn verify_zip(zip_path: &Path, manifest: &ExecutionManifest) -> bool {
    ...
    for required in &manifest.required_files {
        let name_in_zip = format!("{}/{}", manifest.project_root, required.trim_start_matches('/'));
