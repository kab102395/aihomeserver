...
impl Tool for WebSearchTool {
    ...
    async fn execute(&self, params: Value) -> ToolResult {
        let query = match params.get("query").and_then(|v| v.as_str()) {
            ...
            _ => return ToolResult::err(ErrorType::Tool, "missing_param", "query parameter required"),
        };
        ...
        if !results.is_empty() {
            return ToolResult::ok(json!({ "query": query, "results": results, "source": "ddg" }), None);
        }
        ...
        if !lite_results.is_empty() {
            return ToolResult::ok(json!({ "query": query, "results": lite_results, "source": "ddg_lite" }), None);
        }

        // DDG lite also empty — try Bing as last resort
        let bing_url = format!("https://www.bing.com/search?q={encoded}&count=6");
        let bing_html = match self.client
            .get(&bing_url)
        ...
        if !bing_results.is_empty() {
            return ToolResult::ok(json!({ "query": query, "results": bing_results, "source": "bing" }), None);
        }
        ...
        if let Some(reddit_results) = self.reddit_search(&query, &encoded).await {
            if !reddit_results.is_empty() {
                return ToolResult::ok(json!({ "query": query, "results": reddit_results, "source": "reddit" }), None);
            }
        }
        ...
        if let Some(wiki_results) = self.wikipedia_search(&query, &encoded).await {
            if !wiki_results.is_empty() {
                return ToolResult::ok(json!({ "query": query, "results": wiki_results, "source": "wikipedia" }), None);
            }
        }
...
impl WebSearchTool {
    ...
    async fn reddit_search(&self, _query: &str, encoded: &str) -> Option<Vec<Value>> {
        ...
        let url = format!(
            "https://www.reddit.com/search.json?q={encoded}&sort=top&t=year&limit=8",
        );
        ...
        let results: Vec<Value> = posts.iter().filter_map(|p| {
            let d = p.get("data")?;
    ...
    async fn wikipedia_search(&self, _query: &str, encoded: &str) -> Option<Vec<Value>> {
        ...
        let results: Vec<Value> = hits.iter().filter_map(|h| {
...
impl WebSearchTool {
    ...
    async fn searxng_search(&self, query: &str, base_url: &str) -> ToolResult {
        ...
        let raw = match body.get("results").and_then(|v| v.as_array()) {
            Some(a) => a,
            None    => return ToolResult::err(ErrorType::Tool, "no_results", "SearXNG returned no results array"),
        };

        let results: Vec<serde_json::Value> = raw.iter().take(6).filter_map(|r| {
        ...
        ToolResult::ok(json!({ "query": query, "results": results, "source": "searxng" }), None)
    }
}
...
fn extract_ddg_results(html: &str) -> Vec<Value> {
    ...
    while results.len() < 6 {
        ...
        let Some(a_pos) = block.find("result__a") else { continue };
        ...
        let snippet = block.find("result__snippet")
            .and_then(|sp| {
                let after = &block[sp..];
                after.find('>').map(|i| {
                    let rest = &after[i + 1..];
...
fn extract_bing_results(html: &str) -> Vec<Value> {
    ...
    while results.len() < 6 {
        ...
        let Some(rel) = html[pos..].find("b_algo") else { break };
        let block_start = pos + rel;
        ...
        let Some(a_pos) = block.find("<a href=\"http") else { continue };
        let a_block = &block[a_pos..block.len().min(a_pos + 500)];
        let href = extract_attr(a_block, "href").unwrap_or_default();
        if href.is_empty() || href.contains("bing.com") || href.contains("microsoft.com") { continue; }
        let title = a_block.find('>').map(|i| {
            let rest = &a_block[i + 1..];
            rest.find("</a>").map(|j| strip_tags(&rest[..j])).unwrap_or_default()
        }).unwrap_or_default();
...
fn extract_ddg_lite_results(html: &str) -> Vec<Value> {
    ...
    while results.len() < 6 {
        let Some(rel) = html[pos..].find("<a rel=\"nofollow\"") else { break };
        let a_start = pos + rel;
...
fn percent_decode(s: &str) -> String {
    ...
    while i < b.len() {
        ...
        if b[i] == b'+' { bytes.push(b' '); } else { bytes.push(b[i]); }
        i += 1;
    }
